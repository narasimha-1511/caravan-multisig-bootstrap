...
export const WalletOperations = () => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(fetchBalances, 30000); // Fetch every 30 seconds
    return () => clearInterval(interval);
  }, [fetchBalances]);

  return (
    <div className='pt-4'>
      <div className="flex justify-between items-center mb-4 bg-slate-900 p-4 rounded-lg">
        <span className="text-2xl font-medium">Balance: {formatBitcoinAmount({ btc: addresses.reduce((sum, addr) => sum + (addr.balance || 0), 0), sats: 0 })} BTC</span>
        <button
